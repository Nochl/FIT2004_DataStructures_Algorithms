...
def findLongestFromACell(i, j, mat, dp):
    ...
    u, d, l, r, ul, ur, dl, dr = -1, -1, -1, -1, -1, -1, -1, -1
    ...
    if (j < n - 1 and ((mat[i][j] + 1) == mat[i][j + 1])):
        q = 1 + findLongestFromACell(i, j + 1, mat, dp)

    if (j > 0 and (mat[i][j] + 1 == mat[i][j - 1])):
        e = 1 + findLongestFromACell(i, j - 1, mat, dp)

    if (i > 0 and (mat[i][j] + 1 == mat[i - 1][j])):
        a = 1 + findLongestFromACell(i - 1, j, mat, dp)

    if (i < n - 1 and (mat[i][j] + 1 == mat[i + 1][j])):
        d = 1 + findLongestFromACell(i + 1, j, mat, dp)
